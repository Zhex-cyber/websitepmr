                ...
                <div class="row news-details-page-inner">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="news-img-holder">
                            <img src="{{asset('storage/images/berita/' .$berita->thumbnail)}}" class="img-responsive" alt="{{$berita->title}}">
                            <ul class="news-date1">
                                <li>{{Carbon\Carbon::parse($berita->created_at)->format('d M')}}</li>
                                <li>{{Carbon\Carbon::parse($berita->created_at)->format('Y')}}</li>
                            </ul>
                        </div>
                        <h2 class="title-default-left-bold-lowhight">{{$berita->title}}</h2>
                    ...
                    <div class="sidebar-box-inner">
                        ...
                        <ul class="sidebar-categories">
                            @foreach ($kategori as $kategoris)
                                <li><a href="{{ route('kategori.berita', $kategoris->slug) }}"> {{$kategoris->nama}} </a></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
                                ...
                                @foreach ($beritaOther as $beritas)
                                    <li>
                                        <div class="latest-research-img">
                                            <a href="{{route('detail.berita', $beritas->slug)}}"><img src="{{asset('storage/images/berita/' .$beritas->thumbnail)}}" class="img-responsive" alt="{{$beritas->title}}"></a>
                                        </div>
